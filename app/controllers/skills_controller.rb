class SkillsController < ApplicationController
  ...
  def index
    @skills = Skill.page(params[:page]).per(15)
    if params[:searchkey]
      @skills = Skill.where('name like ?',"%#{URI::decode(params[:searchkey])}%").page(params[:page]).per(15)
    end
  end
  ...
  def skill_params
    params.require(:skill).permit(:skillcla_id, :name, :skillimg)
  end
end
